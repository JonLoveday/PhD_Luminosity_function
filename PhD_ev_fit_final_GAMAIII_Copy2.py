...
def ev_fit(infile, outfile, mlims=(0, 19.65), param='m_r',
           Mmin=-24, Mmax=-14, Mbin=40, dmlim=2,
           ...
           lf_est='weight', area=230.6, kc_responses=['galex_FUV', 'galex_NUV', 'vst_u', 'vst_g', 'vst_r', 'vst_i', 'vista_z', 'vista_y', 'vista_j', 'vista_h', 'vista_k', 'wise_w1', 'wise_w2'], r_band_index=4, p = (22.42, 2.55, 2.24)):
    ...
    par.update({'infile': infile, 'mlims': mlims,
                ...
                'idebug': idebug, 'method': method, 'lf_est': lf_est, 'area': area, 'kc': kc, 'r_index': r_band_index, 'p': p})
...
def simcat(infile,
           ...
           Mrange=(-24, -14), mrange=(10, 19.65), zrange=(0.002, 0.65), nz=65,
           fbad=0.03, do_kcorr=True, area_fac=1.0, nblock=500000, schec_nz=0, area=230.6, 
           kc_responses=['galex_FUV', 'galex_NUV', 'sdss_u0', 'sdss_g0', 'sdss_r0', 'sdss_i0', 'vista_z', 'vista_y', 'vista_j', 'vista_h', 'vista_k', 'wise_w1', 'wise_w2'], r_band_index=4):
    ...
    omega_l = 0.7
    par['z0'] = 0
    area = area_fac * area * (math.pi/180.0)**2
...
#    rmin, rmax = cosmo.dm(zrange[0]), cosmo.dm(zrange[1])
    ...
    if do_kcorr:
        sel = ((tbdata['SC'] >= 7) *
               ...
               (tbdata['NQ'] > 2))
        for ic in range(5):
            sel *= np.isfinite([x[ic] for x in tbdata['pcoeffs']])
        tbdata = tbdata[sel]
...
#         plt.plot(zbin + par['z0'], k_median)
    ...
    if schec_nz:
        zlims = np.linspace(0.0, zrange[1], nz+1)
        nz = len(zlims) - 1
...
class Sample(object):
    ...
    def __init__(self, infile, selpar, sel_dict, Q=0, chi2max=10, nqmin=2):
...
#             print(nclean, 'out of', ncand, 'targets with clean photometry')
        ...
        if mock:
            ...
            gal_arr['z'] = z
            gal_arr['kc'] = tbdata['kcorr_r'] #kcorrection value
            gal_arr['kcoeff'] = tbdata['pcoeff_r'] #pcoeffs
            gal_arr['coeff'] = tbdata['coeff_r']
        ...
        good = np.ones(len(gal_arr), dtype=bool)
...
class Cost(object):
    ...
    def __init__(self, samp, nz, xxx_todo_changeme, lf_bins, lf_zbins, 
                 method, P_prior, Q_prior, Qmin, Qmax, err_type='jack'):
        (zmin, zmax) = xxx_todo_changeme
        self.samp = samp
        self.nz = nz
        self.zmin = zmin
        self.zmax = zmax
...
#                     len(self.zhist[idx]), converged, niter))
            ...
            if math.isnan(self.chisq):
                print('NAN CHI^2, PRINTING OUT VALUES FOR DEBUGGING')
                print('densum', densum)
                print('self.zhist[idx]', self.zhist[idx])
                print('self.V[idx]', self.V[idx])
                print('Pz[idx]', Pz[idx])
                print('self.delta[idx]', self.delta[idx])
                print('phisum', phisum)
                print('self.whist', self.whist)
                print('self.phi', self.phi)
                print('xsum', xsum)
                print("self.gala['weight']", self.gala['weight'])
                print('sum2', sum2)
                print('len(self.gala)', len(self.gala))
                print('self.delta[idx]', self.delta[idx])
                print('del_var[idx]', del_var[idx])
                print('self.P', self.P)
                print('self.P_prior[0]', self.P_prior[0])
                print('self.P_prior[1]', self.P_prior[1])
                print('self.Q', self.Q)
                print('self.Q_prior[0]', self.Q_prior[0])
                print('self.Q_prior[1]', self.Q_prior[1])
                print('self.chisq', self.chisq)
                self.chisq = 1e9
...
def delta_solve(P, Q, gala, nz, xxx_todo_changeme3, zbin, zhist, V, V_int, S, 
                nitermax=50, delta_tol=1e-4):
    """Solve for overdensity delta for given P, Q."""
    (zmin, zmax) = xxx_todo_changeme3
    converged = False
    niter = 0
    Npred = np.zeros(nz)
